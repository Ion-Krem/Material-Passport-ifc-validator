# Material Passport IFC Validator

Standalone Python CLI tool that validates IFC (BIM) files against [Madaster](https://madaster.com) platform requirements before upload. Checks classification codes, base quantities, materials, and property sets — outputs HTML + Excel reports for BIM coordination.
...
```bash
# Clone the repo
git clone https://github.com/YOUR_USERNAME/madaster-ifc-validator.git
cd madaster-ifc-validator
...
pyinstaller --onedir --name MadasterValidator \
...
The executable will be in `dist/MadasterValidator/`.
